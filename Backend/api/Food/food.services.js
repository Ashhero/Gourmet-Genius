...
module.exports  = {
     ...
     getDatas:(callBack) => {
         ...
         pool.query(
            ...
            // `,
            `SELECT fr.id, n.first_name, b.program, b.program_date, fr.request_status, 
            fr.request_date FROM food_request fr JOIN users n ON fr.ngo_id = n.id JOIN booking b 
            ON fr.booking_id = b.id`,
            (err,results) => {
                if(err){
                    return callBack(err);
                }else if(results == ""){
     ...
     getRequested:(callBack) => {
        ...
        pool.query(
           `SELECT fr.id, n.first_name, b.program, b.program_date, fr.request_status, fr.request_date
           FROM food_request fr
           JOIN users n ON fr.ngo_id = n.id
           JOIN booking b ON fr.booking_id = b.id
           `,
           (err,results) => {
               if(err){
